...
`quick_validate.py` 对 7 个 Skill pack 做结构、发布清单、脚本语法、错误输入和最小运行烟测；`security_scan.py` 对 6 个子 Skill 脚本做确定性静态安全门。第三方 skills.sh／Socket 审计只有在扫描快照包含当前脚本时才算当前收据，详细边界见 [docs/SECURITY.md](docs/SECURITY.md)。
