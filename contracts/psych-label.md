...
仅在当前回合出现与用户目标直接相关的明确证据时加载：

- 用户直接承认自己知道该做但没有做、正在逃避或拖延；或
...
| 标签 | 使用条件 |
|---|---|
| `USER_ADMITS` | 用户原话直接承认心理卡点，例如“我知道该做但就是不做”“我是在逃避”“我就是拖延”。必须保留可回读的原话或私密证据引用。 |
| `BEHAVIORAL_CONTRADICTION` | 同一目标、同一时间范围内，用户自述与具体行为满足至少一条明确标准，并且没有已知的合理背景可以解释该不一致。 |
| `SUSPECTED` | 有语言或行为线索，但证据不足以达到前两类；不能写成确定性结论。 |
