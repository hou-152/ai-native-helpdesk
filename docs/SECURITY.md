...
- 禁止子进程、动态代码执行、反射式属性／原型访问、无法静态解析的解构 key、计算值通过 `throw`／`catch` 流转、从函数体 hash 未批准的 primitive reader 返回、传入非批准数据消费者或进入 class field，以及可执行 constructor 链（含常量别名、字符串拼接、静态数组 `join`，以及直接、连续计算成员、默认值、对象属性写入、callback 参数或经普通／解构变量传播的计算调用目标）、环境变量读取和文件写入；`process` 只允许直接访问 `argv`、`stdout`、`stderr`、`exitCode`，禁止别名逃逸、计算属性和禁用 re-export；
