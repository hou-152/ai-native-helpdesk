...
- 禁止子进程、动态代码执行、反射式属性／原型访问、计算值从非批准 primitive reader 返回，以及可执行 constructor 链（含常量别名、字符串拼接、静态数组 `join`，以及直接、连续计算成员、默认值、对象属性写入或经普通／解构变量传播的计算调用目标）、环境变量读取和文件写入；`process` 只允许直接访问 `argv`、`stdout`、`stderr`、`exitCode`，禁止别名逃逸、计算属性和禁用 re-export；
