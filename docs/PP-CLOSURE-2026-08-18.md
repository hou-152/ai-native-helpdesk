...
|---|---|---|---|
| ① 机制完成 | 8 卡、198／198、可逆安装、Draft PR 可审查 | `COMPLETE / DECLARABLE` | PP 已由此关门 |
| ② merge／Release | Draft PR → ready → merge `main` → tag／GitHub Release | `DRAFT_PR_OPEN / PENDING_OWNER_AUTHORIZATION` | 否；每一步仍须单独授权 |
...
- ② `MERGE_RELEASE`：Draft PR #5 已打开；ready、merge、tag、GitHub Release 均待逐项授权。
- ③ `PRODUCT_VALIDATION_30`：发布后阶段；目标 15／30，尚无冻结查询集与真人实测，结果 `UNKNOWN`。
- 远端 `main` 仍只有 1 张 PublicCard；这不回退 PP 机制关门，也不能被误写成 8 卡已正式发布。
